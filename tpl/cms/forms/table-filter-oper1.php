        ...
        foreach ($TPL['TABLEFORM'] as $b)
        {
            if ($b['mid'] != $cur_m)
            {
                ...
                { ?>
                    ...
                    <input type="submit" value="Распечатать протокол оценки заявок"><br> <?
                }
                if ($b['sid'] != $cur_sp)
                {
                    ...
                    $cur_sp = $b['sid'];
                    $i_b_in_sp = 0; ?>
                    <h3><b>Подпрограмма "<?=$b['stitle']; ?>"</b></h3>
                    Всего найдено <b><span id="i_b_in_sp_<?=$b['sid']?>"></span></b> заявок <?
                } ?>
                <h4>Мероприятие "<?=$b['mtitle']; ?>"</h4> <?
                $me_id = str_replace(".", "a", $cur_m); ?>
