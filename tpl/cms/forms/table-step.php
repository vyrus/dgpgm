...
dojo.ready(function()
{
    ...
    <?
    $postfix = $bid_id."_".$TPL['STEPSDATA'][0]['id'];
    ?>
    dojo.query("select.step_date").connect("onchange",remakeRestCombos);



    makeCombobox("startmonth_<?=$postfix?>",<?=$TPL['STARTDATEARR'][1]+1?>,<?=$TPL['STARTDATEARR'][1]+1?>,"wellSel",<?=$TPL['STARTDATEARR'][0]?>);
    var range = [];
    for (var i=<?=$TPL['STARTDATEARR'][1]+2?>; i<=12; i++)
    {
        range.push(i);
    }
                ...
                st_box_cont = '<h4>Этап '+data.step_number+'&nbsp;<span class="local_red">не все поля этапа заполнены</span>&nbsp; <img border="0" src="/adm/icon/delete_16.png" style="cursor:pointer" class="del_icon"></h4>'+
                    ...
                    '<table width="100%">'+
                      ...
                      '<tr>'+
                        ...
                        '<td><input type="text" name="finish_month" value=""></td>'+
                      '</tr>'+
                      '<tr>'+
...
yearA.push(this);
console.info(yearA);
year = yearA.data('year');
console.info(year);
nextStepNum =2;
curStepFinishMonthVal = this.options[this.selectedIndex].value;
...
for (var st=nextStepNum;st<list.length;st++)
{
    ...
    nextSelStart = dojo.query(">select","#".nextSelStartContainerId)[0]; //селект стартового месяца в следующем этапе
    ...
    nextSelFinish = dojo.query(">select","#".nextSelFinishContainerId)[0]; //селект финишгого месяца в следующем этапе
...
foreach ($TPL['STEPSDATA'] as $step_data)
{
    ...
    if ($year != $cur_year)
    {
        ...
        <script> match_step_num_to_id["<?=$year?>"] = [];
...
<script> match_step_num_to_id["<?=$year?>"].push("<?=$bid_id?>_<?=$step_data['id']?>"); </script>
...
<table width="100%">
  ...
  <tr>
    ...
    dojo.ready(function()
    {
        dojo.query("#startmonth_<?=$bid_id?>_<?=$step_data['id']?>").data("year", <?=$year?>); console.info(dojo.query("#startmonth_<?=$bid_id?>_<?=$step_data['id']?>").data("year"));
    })</script>
  </tr>
  <tr>
    <td>Месяц окончания работ</td>
    ...
    dojo.ready(function()
    {
        dojo.query("#finishmonth_<?=$bid_id?>_<?=$step_data['id']?>").data("year", <?=$year?>);  console.info(dojo.query("#finishmonth_<?=$bid_id?>_<?=$step_data['id']?>").data("year"));
    })</script>
  </tr>
  <tr>
    <td>Стоимость работ, руб</td>
