              ...
              $uplfile['size'] <= $this->getCurrentDirSetting('max_upload_filesize'))
          {
            ...
            */
            ...
            if (!$ok) {
			  ...
			  // file translate for zlyuk
			  $gost = array(
				"ª"=>"eh","²"=>"i","³"=>"i","¹"=>"#","º"=>"eh",
				"À"=>"a","Á"=>"b","Â"=>"v","Ã"=>"g","Ä"=>"d",
				"Å"=>"e","¨"=>"jo","Æ"=>"zh",
				"Ç"=>"z","È"=>"i","É"=>"jj","Ê"=>"k","Ë"=>"l",
				"Ì"=>"m","Í"=>"n","Î"=>"o","Ï"=>"p","Ð"=>"r",
				"Ñ"=>"s","Ò"=>"t","Ó"=>"u","Ô"=>"f","Õ"=>"kh",
				"Ö"=>"c","×"=>"ch","Ø"=>"sh","Ù"=>"shh","Ú"=>"'",
				"Û"=>"y","Ü"=>"","Ý"=>"eh","Þ"=>"yu","ß"=>"ya",
				"à"=>"a","á"=>"b","â"=>"v","ã"=>"g","ä"=>"d",
				"å"=>"e","¸"=>"jo","æ"=>"zh",
				"ç"=>"z","è"=>"i","é"=>"jj","ê"=>"k","ë"=>"l",
				"ì"=>"m","í"=>"n","î"=>"o","ï"=>"p","ð"=>"r",
				"ñ"=>"s","ò"=>"t","ó"=>"u","ô"=>"f","õ"=>"kh",
				"ö"=>"c","÷"=>"ch","ø"=>"sh","ù"=>"shh","ú"=>"",
				"û"=>"y","ü"=>"","ý"=>"eh","þ"=>"yu","ÿ"=>"ya",
				" "=>"_","«"=>"","»"=>"","—"=>"-","\""=>"",
				","=>"","."=>"","?"=>"","!"=>"","("=>"",")"=>"","/"=>"_"
			  );
			  preg_match('#(.*)(\.[a-zA-Z]+)#is',$uplfile_name,$found);
			  $tmp = strtr($found[1], $gost);
			  $uplfile_name = $tmp.$found[2];
			  //end file translate for zlyuk
              $i = 1;
              // pick unused file name
              while (file_exists($this->getCurrentFsDir().$uplfile_name)) {
                $uplfile_name = ereg_replace('(.*)(\.[a-zA-Z]+)$', '\1_'.$i.'\2', $tmp.$found[2]);
                $i++;
              }
