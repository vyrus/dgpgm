...
class ManagerForms{
	...
	function maxBidMeasure($notice_id, $turn) { //$noticeinfo['notice_id']   $noticeinfo['turn']
		$sql=sql_placeholder('select max(b.bid_id_in_measure) as max_id from ?#FK_BID as b, ?#FK_NOTICE as n, ?#FK_MEASURE_HAS_NOTICE as mn where
			...
			and n.turn=?', $notice_id, $turn);
		return $this->db->select_row($sql);
	}
	
	//Информация об одной заявке
	function viewBid($id){
		$sql=sql_placeholder('select b.*, m.title as title_measure, m.subprogram_id, p.title as title_subprogram, YEAR(n.start_realization) as start_realization, n.finish_acquisition, d.title as departament_name, mn.measure_id
...
// список заявок пользователя для меню мои заявки
	function listUserBids($st, $user_id) {
        /* if $st==2 с начала сбора до даты завершения
           if $st==3 с даты завершения до даты подведения итогов
           if $st==4 с даты подведения итогов и >
        */
		if ($st == 1) { // показать заявки не поданные с начала сбора до даты завершения
			$where = " and b.datetime_electron_bid_receiving is null and n.start_acquisition < NOW() and NOW() < n.finish_acquisition";
		} elseif ($st==2) { // показать заявки с даты завершения до даты подведения итогов
			$where = " and b.datetime_electron_bid_receiving is not null and n.start_acquisition < NOW() and NOW() < n.summing_up_date"; //n.finish_acquisition < NOW()
        }elseif ($st==3) { // рассмотренные заявки с даты подведения итогов и >
			$where = " and b.datetime_electron_bid_receiving is not null and n.summing_up_date < NOW()";
        }elseif ($st==4) {
			$where = "";
        }
            ...
            switch(1){
				...
				case($_GET['action']=='mr_get'):
					...
					if ($r) {
						...
						foreach ($r as $row) {
							$measure[] = array('id'=>$row['id'], 'title'=>utf8_str_word($row['title'], 6, ' '));
						}
						$result = array('type'=>'success', 'measure'=>$measure);
					} else {
						$result = array('type'=>'error');
				...
				case($action=='infoyur' && $_GET['id']):// 
					...
					if ($TPL['INFO']['applicant_organization_id'] == 0) {
						header('Location: http://'.$_SERVER['SERVER_NAME'].'/forms/bid/'.$_GET['id'].'/infoyurfirst'); exit;
					}
    ...
    //            				ManagerUser::editUserData(USER_ID, $row);
    						...
    						$sql=sql_placeholder('update ?#FK_BID set work_topic= ?,
                                ...
                                $_TPL['ROW']['finish_date'], $_TPL['ROW']['price_works_actual'], $_GET['id']);
    						$this->db->query($sql);
                        }
...
/*				case($action=='calendplan' && $_GET['id']):
				...
				case($action=='other-price' && $_GET['id']):
					...
					if ($TPL['INFO']['user_id'] == USER_ID) {
						...
						$TPL['INFO']=$this->viewBid(intval($_GET['id']));
						$_TPL['BIDMENU']=$this->createBidMenu($TPL['INFO'], $_TPL['USERDATA'], 'price');
						if ($finish_acquisition_tsh<time() || !empty($TPL['INFO']['datetime_electron_bid_receiving'])) {
							$form_dis='disabled="disabled"';
						} else {
							$form_dis='';
				...
				case($action=='1' && $_GET['id']): // это генерация печатной формы заявки
					...
					include TPL_CMS_FORMS."print-info-fiz.php";
				break;				
				
				case($action=='my-bids'):
					$st = intval($_GET['st']);
					$_TPL['MYBIDS'] = $this->listUserBids($st, USER_ID);
					if (!empty($_TPL['MYBIDS'])) {
						$result = array('type'=>'success', 'value'=>$_TPL['MYBIDS']);
					} else {
						$result = array('type'=>'error');
					}
					print json_encode($result);
				...
				case($action=='duplicate-org' && $_GET['id']):
					$org = intval($_GET['id']);
					//в заявку вписываем applicant_organization_id из таблицы юзер (КОШМАР!!!!)
					$bid_info['applicant_organization_id'] = $_TPL['USERDATA']['id_org_ind'];
					$this->updateBid($_TPL['USERDATA']['bid_id'], $bid_info);
					// получаем информацию об организации с $_GET['id']
					$TPL['APPLICANT']=$this->viewOrganization($org);
					unset ($TPL['APPLICANT']['id']);
					// вставляем этот массив в таблицу с организациями с айди applicant_organization_id
					$sql=sql_placeholder('update ?#FK_APP_ORG set ?% where id=? ', $TPL['APPLICANT'], $_TPL['USERDATA']['id_org_ind']);
                    $this->db->query($sql);
					// находим значение complete в таблице пользователей для этой организации .......
					$sql=sql_placeholder('select complete_info from ?#FK_USER where id_org_ind=? ', $org);
				...
				case($action=='print' && $_GET['id']): // это генерация печатной формы заявки
					...
					if (USER_TYPE == 'yur') {
						...
						$applicant_form = $TPL['APPLICANT']['director_duty']." ".$TPL['APPLICANT']['full_title_genitive'];
					...
					if ($TPL['INFO']['user_id'] == USER_ID) {
						...
						if ($finish_acquisition_tsh>time()) {
						...
						//первая страница
							ob_start();
							include TPL_CMS_FORMS."print-form1.php";
						...
						// вторая страница
							ob_start();
							if (USER_TYPE == 'yur') {
								include TPL_CMS_FORMS."print-info-yur.php";
							} elseif (USER_TYPE == 'fiz') {
								include TPL_CMS_FORMS."print-info-fiz.php";
							}
							$applicant=ob_get_contents();
							ob_end_clean();
...
$pdf = new TCPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
						...
						//$pdf->AddPage();
						$pdf->writeHTML($form1, true, false, true, false, '');
						$pdf->EndPage();
						//$pdf->lastPage();
				...
				case($action=='tablestep'):
					...
					if ($TPL['INFO']['user_id'] == USER_ID)
                    {
                        $TPL['STARTDATEARR'] = split("-",$TPL['INFO']['start_date']);
                        $TPL['FINISHDATEARR'] = split("-",$TPL['INFO']['finish_date']);
                        if (empty($TPL['INFO']['start_date']) || empty($TPL['INFO']['finish_date'])) {include TPL_CMS_FORMS."redirect-tz.php";exit;}
    					$start = $TPL['STARTDATEARR'][0];
    					$end = $TPL['FINISHDATEARR'][0];
                            ...
                            $complete = $this->checkStepIsComplete($_POST);
                      		$sql=sql_placeholder('update ?#FK_WORK_STEP set report_documentation_composition=?,
                                                  ...
                                                  where bid_id=? and id=?',
                                                        ...
                                                        $_POST['cost'],
                                                        $_POST['start_month'],$_POST['finish_month'], $complete, $bid_id, $step_id);
                    		$this->db->query($sql);
                        }
            			// обновить заполненость формы
                        $stepWorksForBid = $this->stepWorksByBid($bid_id);
                        foreach($stepWorksForBid as $stp)
                        {
                          if ($stp['complete']!=1)
                            {
                                $this->tableComplete($_GET['id'], 'tablestep', 0); break;
                            } else {$this->tableComplete($_GET['id'], 'tablestep', 1);}
                        }
				...
				case($action=='newwork'):
                    ...
                    {
                        if (empty($work['description'])) {$jsonworkslist .= "{id: ".$work['id'].", title: '".addslashes($work['title'])."', done: 0},";}
                        else {$jsonworkslist .= "{id: ".$work['id'].", title: '".addslashes($work['title'])."', done: 1},";}
                    }
				...
				case($action=='getwork'):
    				...
    				if (isset($work_id))
                    {
                        ...
                        $jsonworkdata = "{title: '".addslashes($wd['title'])."', description: '".addslashes($wd['description'])."', work_id: '".$work_id."'}";
                        echo $jsonworkdata;
                    }
				break;
				...
				case($action=='printandsubmit' && $_GET['id']):
					//получаем информацию о заявке
					$TPL['INFO']=$this->viewBid($_GET['id']);
					if ($TPL['INFO']['user_id'] == USER_ID) {
						...
						if ($_POST['post-elect-bid']) {
							...
							$this->updateBid($_GET['id'], $bid_info);
							exit;
							//header('Location: /forms/bid/'.$_GET['id'].'/printandsubmit');
						}
						...
						if (!empty($TPL['INFO']['datetime_electron_bid_receiving'])) {
							...
							$form_print_name = 'Распечатать черновой вариант заявки';					
						}
						
					include TPL_CMS_FORMS."printandsubmit.php";
					}else{
					include TPL_CMS_FORMS."no-rights.php";
					}
				break;
	...
	function nameform($form_type) {
		if ($form_type == 'infofiz') {
			$form_name = "Данные о заявителе";
		} elseif  ($form_type == 'infoyur') {
			$form_name = "Данные о заявителе";
		} elseif  ($form_type == 'tz') {
			$form_name = "Техническое задание";
		} elseif  ($form_type == 'tablestep') {
			$form_name = "Календарный план";
    ...
    function checkStepIsComplete($step_data)
    {
        if (!empty($step_data['report_documentation_composition']) &&
            ...
            !empty($step_data['start_month']) &&
            !empty($step_data['finish_month']) ) {return true;} else {return false;}
    }
...
/*конец адресам*/
	...
	function createBidMenu($row_bid_info, $user_info, $action) {
		$menu = '<ul class="tabs">';
		$menu .= '<li class=""><img border="0" src="/adm/icon/done.png"> Шаг 1<br />Регистрация</li>'; // регистрация неактивна всегда
		
		// неактивны, если: (Заявка отправлена) или (истекло время подачи)
		$finish_acquisition_tsh = strtotime($row_bid_info['finish_acquisition']);
		if (!empty($row_bid_info['datetime_electron_bid_receiving']) || $finish_acquisition_tsh<time()) { // заявка отправлена уже или время истекло
			$edit = false;
		} else {
			$edit = true;
		}
		$bid_action = 'info'.$user_info['type-face'];
		if ($action != $bid_action ) {
			$href = "<a href='/forms/bid/".$row_bid_info['id']."/".$bid_action."'>";
			$endhref = "</a>";
			$class = "hover";
		} else {
			$href = "";
		...
		if ($user_info['complete_info']==1) {
			$img = "<img border='0' src='/adm/icon/done.png'> ";
		} else {
			$img = "<img border='0' src='/adm/icon/attension.png'> ";
		}
		if (!$edit) {
			$href = "";
			$endhref = "";
		}
		// сюда данные о заявителе надо вставить....
		$menu .= '<li class="'.$class.'">'.$href.$img.'Шаг 2<br />Данные о заявителе'.$endhref.'</li>';
		// шаги 3-6:
		$TPL['COMPLETE']=$this->formsComplete($row_bid_info['id']);
		$step = 3;
		$compl = 0;
		foreach ($TPL['COMPLETE'] as $row) {
			if ($action != $row['form_type']) {
				$href = "<a href='/forms/bid/".$row_bid_info['id']."/".$row['form_type']."'>";
				$endhref = "</a>";
				$class = "hover";
			} else {
				$href = "";
			...
			$menu .= '<li class="'.$class.'">'.$href.$img.'Шаг '.$step.'<br />'.$this->nameform($row['form_type']).$endhref.'</li>';
		$step++;
		}
		if (!empty($row_bid_info['datetime_electron_bid_receiving'])) {
			$img = "<img border='0' src='/adm/icon/done.png'> ";
			$class = "";
			$href = "";
			$endhref = "";
			...
			$img = "<img border='0' src='/adm/icon/attension.png'> ";
			$class = "hover";
			$href = "<a href='/forms/bid/".$row_bid_info['id']."/printandsubmit'>";
			$endhref = "</a>";			
		} else {
			$img = "";
			$class = "";
		...
		$menu .= '<li class="'.$class.'">'.$href.$img.'Шаг 6<br />Печать и отправка'.$endhref.'</li>';
