 ...
 if (!empty($_FILES[$fname]) && is_uploaded_file($_FILES[$fname]["tmp_name"]))
    {
            ...
            if($max_size && $size>$max_size) return array('Размер файла больше '.$max_size);

        if (empty($_FILES[$fname]["tmp_name"])){
            return array('Ошибка загрузки файла. Код '.$_FILES[$fname]["error"]);
        }
        ...
        if ((in_array($type,$allowed_types))){
            ...
            if ($res){
              ...
              return array('Ошибка загрузки файла');
            }
        }else{
           return array('Не верный тип файла. Допустимо:' .implode(',',$allowed_types));
       }
    }
    else
    {
      return array('Файл не был загружен');
    }
}
 ...
 if (!empty($_FILES[$fname]) && is_uploaded_file($_FILES[$fname]["tmp_name"]))
    {
        ...
        if ((in_array($type,$allowed_types))){
           ...
           return array('Не верный тип файла. Допустим формат zip');
       }
    }
    else
    {
